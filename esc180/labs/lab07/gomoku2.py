...
def is_bounded(board, y_end, x_end, length, d_y, d_x):
    y_start, x_start = y_end - length * d_y, x_end - length * d_x
    start_bound = (0 <= y_start < len(board)) and (0 <= x_start < len(board[0])) and (board[y_start][x_start] != " ")
    end_bound = (0 <= y_end < len(board)) and (0 <= x_end < len(board[0])) and (board[y_end][x_end] != " ")
...
def detect_row(board, col, y_start, x_start, length, d_y, d_x):
    ...
    if not (0 <= y_end < len(board)) or not (0 <= x_end < len(board[0])):
    ...
    bound = is_bounded(board, y_end, x_end, length, d_y, d_x)
    if bound == "OPEN":
        open_seq_count += 1
    elif bound == "SEMIOPEN":
